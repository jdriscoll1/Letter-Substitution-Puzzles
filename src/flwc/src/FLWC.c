...
int botPly_MaxAdjacencies(int word, struct WordSet* goalWords, struct DataStructures* data){
	...
	/* if the word is used, or is one this board may not play, continue.
	...
	if(goalWords != NULL){
		...
		//FLWG_LOG("Curr Word In Goal Set: %s\n", (currWordInGoalWordSet) ? "true" : "false"); 
		...
		if(currWordInGoalWordSet && !optionInGoalWordSet){
			//FLWG_LOG("Curr Word is in the Goal and the Option is Not, Choosing %d\n", optionId);
			resultId = optionId; 
			options = options->next; 
			continue; 
		}
		if(!currWordInGoalWordSet && optionInGoalWordSet){
			//FLWG_LOG("Curr Word is not in the Goal Set and the Option is, Selecting Current Word\n");
			options = options->next; 
			continue; 
		}
		//FLWG_LOG("The Current Word is not in the goal set OR the Option is in the Goal Set, Moving Forward\n");
	...
	// If the Potential Word Has More Options Than the Current
		...
		//FLWG_LOG("The Current Word Has More Connections than the option, NOT SELECTING THE OPTION \n"); 
		options = options->next; 
		continue; 

	}
	// if all parameters exceed, choose the result
	//FLWG_LOG("Success! Selecting The Option"); 
	resultId = optionId; 
        options = options->next;
